...
/**
 * Map event type → label for event type
 * @type Map<string, string>
 */
const plEventsToLabels = new Map([
...
export default class RolesRoomSettingsTab extends React.Component {
    ...
    _populateDefaultPlEvents(eventsSection, stateLevel, eventsLevel) {
        for (const desiredEvent of Array.from(plEventsToShow.keys())) {
            if (!(desiredEvent in eventsSection)) {
                eventsSection[desiredEvent] = (plEventsToShow.get(desiredEvent).isState ? stateLevel : eventsLevel);
            }
        }
    }
    ...
    render() {
        ...
        const eventPowerSelectors = Object.keys(eventsLevels).map((eventType, i) => {
            let label = plEventsToLabels.get(eventType);
            if (label) {
                label = _t(label);
            } else {
